...
import chapter13.bean.{Data, Message, Result, Task}
...
//            println("客户端已经连接。。。。。。。")
            // TODO 当获取连接后应该使用新的线程进行处理，而不是在当前线程处理，会影响效率
            new Thread(
              new Runnable {
                override def run(): Unit = {
...
//                      println("接收到用户提交的作业：" + dat)
  ...
  //                      data.slice()   // 切分数据
                      val iterator = data.sliding(perTaskDataCnt, perTaskDataCnt) // 滑动窗口
                      var cnt = 0
