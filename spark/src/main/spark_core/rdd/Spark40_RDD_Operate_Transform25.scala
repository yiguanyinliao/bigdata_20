...
object Spark40_RDD_Operate_Transform25 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //  小练习：将数据List(("a", 88), ("b", 95), ("a", 91),
    //  ("b", 93), ("a", 95), ("b", 98))求每个key的平均值。
    ...
    //    第一个参数表示：为了方便统计，将 （第一个） key出现的值转换结构
    //    第二个参数表示：分区内的计算规则
    //    第三个参数表示：分区间的计算规则
    //
    ...
    newRDD1.mapValues(t => {t._1/t._2}).collect().foreach(println)
...
//    rdd.aggregateByKey()
    ...
    // TODO reduceByKey可以实现两两聚合，但是要求处理的数据类型和value的数据类型保持一致
//    rdd.reduceByKey()

    // TODO groupByKey可以实现功能，性能太低，不推荐使用
